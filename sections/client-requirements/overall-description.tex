...
XZES40-Transformer ought to have two user interfaces:
...
\begin{description}
  \item {
    ...
    \begin{description}
      ...
      \item {
        [\inlinecode{--output-file=}] (optional)
        Specifies a file to write out to.  Otherwise writes to \inclinecode{STDOUT}.
      }
      \item {
        [\inlinecode{--port=}] (optional)
        Specifies which port to connect through if non-standard (\inlinecode{e.g., 8001})
      }
      \item {
        [\inlinecode{--username=}] (optional)
        Specifies a user name if one is required.
      }
      \item {
        [\inlinecode{--password=}] (optional)
        Specifies the user's password if one is required.
      }
      \item {
        [\inlinecode{--help=}] (optional)
...
It will be assumed that the user of our application uses our application on a fairly modern monitor.
